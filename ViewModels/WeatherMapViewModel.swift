...
// MARK:  add your appid in the url below:
        if let url = URL(string: "https://api.openweathermap.org/data/3.0/onecall?lat=\(lat)&lon=\(lon)&units=metric&appid=6ca31a03de83a6312e577e5c96473bbb") {
            ...
            do {
                ...
                print(weatherDataModel.current)
    ...
    func loadLocationsFromJSONFile(cityName: String) -> [Location]? {
        if let fileURL = Bundle.main.url(forResource: "places", withExtension: "json") {
            do {
                ...
                let locations = allLocations.filter { $0.cityName == city }
                return locations
            } catch {
